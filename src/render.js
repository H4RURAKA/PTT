...
export function buildTeamBoard(teamBoard, team, TYPES, TYPE_COLOR, TYPE_LABEL){
  ...
  for (let i=0;i<6;i++){
    ...
    row.className='slot'; 
    row.dataset.i=i;
    row.classList.add('no-sprite');   // ← 기본은 스프라이트 없음 상태
    
    row.innerHTML = `
      <div class="top">
        <h3>포켓몬 ${i+1}</h3>
        <div class="actions"><button class="btn" data-clear="${i}">초기화</button></div>
      </div>
    ...
    TYPES.forEach(([k,ko])=>{
      const chip = document.createElement('div'); chip.className='chip'; chip.dataset.key=k; chip.dataset.i=i;
      chip.innerHTML = `<span class="dot" style="background:${TYPE_COLOR[k]}"></span><span>${ko}</span>`;
      tray.appendChild(chip);
    });

    row.querySelector('input.name').value = team[i].name || '';
  }
}

export function reflectChips(teamBoard, team, i){
  const row = teamBoard.querySelector(`[data-i='${i}']`); if (!row) return;
  const cur = team[i];
...
function tagKo(tag){
  const map = { physical:'물리', special:'특수', wall:'막이', hazard:'깔개', speed:'스피드', balanced:'밸런스', tank:'탱크' };
  return map[tag] || tag;
}

export function renderSlotMini(teamBoard, team, i, ATK, TYPES, TYPE_COLOR, TYPE_LABEL, combinedDefenseMultiplier){
  const mini = teamBoard.querySelector(`[data-mini='${i}']`); if (!mini) return;
  mini.innerHTML = '';
  ...
  if (pkm){
    ...
    metaRow.appendChild(pill(`HP: ${HP} | ATK: ${ATKv} | SPATK: ${SPATK} | DEF: ${DEF} | SPDEF: ${SPDEF} | SPD: ${SPD}`, 'stat-pill'));
  } else {
  ...
  for (const [atk] of TYPES){
    ...
    else if (m === 0.5) half.push(atk);
    else if (m === 0.25) quarter.push(atk);
  }

  // === 2줄: [약점] [4×] types … [2×] types … ===
  const weakRow = document.createElement('div'); weakRow.className='weak-row';
  ...
  // === 3줄: [반감] [0.5×] types … [0.25×] types … ===
  const resistRow = document.createElement('div'); resistRow.className='weak-row';
  ...
  function pill(text, extraClass=null, raw=false){
    const el=document.createElement('span'); el.className='pill' + (extraClass?` ${extraClass}`:'');
    el.innerHTML = raw ? text : escapeHtml(text); return el;
  }
  function pillWithDot(text,color){
    const el=document.createElement('span'); el.className='pill';
    el.innerHTML = `<span class="dot" style="background:${color}"></span>${text}`; return el;
  }
...
export function renderAttackSummary(atk, TYPE_LABEL, TYPE_COLOR, listEl, strongCountEl, holeCountEl, typesCountEl){
  ...
  typesCountEl.textContent = atkTypes.length.toString();
  strongCountEl.textContent = `${strongDefs.length} / 18`;
  holeCountEl.textContent = (18 - strongDefs.length).toString();
  listEl.innerHTML = '';
...
export function renderAttackTable(tableEl, TYPES, TYPE_KEYS, TYPE_LABEL, ATK, effect, effClass, bestByDef){
  const thead = `<thead><tr>${
    ['공격\\방어', ...TYPES.map(([,ko])=>ko)].map((h,idx)=> idx===0? `<th class="sticky-col">${h}</th>` : `<th class="col-${TYPE_KEYS[idx-1]}">${h}</th>`).join('')
  }</tr></thead>`;
  const bestTds = [`<td class="sticky-col"><b>전체</b></td>`];
  for (const [def] of TYPES){ const m=bestByDef[def]??1; bestTds.push(`<td class="col-${def}"><span class="eff ${effClass(m)}">${m}×</span></td>`); }
...
export function renderAttackRows(tableEl, atkTypes, TYPES, TYPE_LABEL, ATK, effect, effClass){
  const tbody = tableEl.querySelector('tbody'); const frag=document.createDocumentFragment();
  for (const atk of atkTypes){
    const tds = [`<td class="sticky-col"><b>${TYPE_LABEL[atk]}</b></td>`];
    for (const [def] of TYPES){ const m = effect(ATK, atk, def); tds.push(`<td class="col-${def}"><span class="eff ${effClass(m)}">${m}×</span></td>`); }
    const tr = document.createElement('tr'); tr.innerHTML = tds.join(''); frag.appendChild(tr);
  }
...
export function applyAttackHighlight(tableEl, TYPES, bestByDef){
  TYPES.forEach(([k])=>{
    ...
    tableEl.querySelectorAll(`.col-${k}`).forEach(cell => cell.classList.toggle('hole-col', hole));
    const th = tableEl.querySelector(`thead th.col-${k}`); if (th) th.classList.toggle('hole-col', hole);
  });
}

export function renderHoleSuggestions(panelEl, TYPES, TYPE_LABEL, TYPE_COLOR, coverTypesForFn, bestByDef){
  panelEl.innerHTML=''; const holes = TYPES.filter(([k])=>(bestByDef[k]||1)<=1).map(([k])=>k);
  if (!holes.length){ panelEl.style.display='none'; return; }
  panelEl.style.display='';
  holes.forEach(def=>{
    const block=document.createElement('div'); block.className='hole-block';
    const title=document.createElement('span'); title.className='pill'; title.innerHTML=`<b>${TYPE_LABEL[def]} 커버:</b>`;
...
export function renderDefenseTable(tableEl, TYPE_LABEL, agg){
  const thead=`<thead><tr>
    <th class="sticky-col">공격 타입</th><th>4×</th><th>2×</th><th>1×</th><th>0.5×</th><th>0.25×</th><th>0×</th>
  </tr></thead>`;
  const rows=[];
...
export function renderRemaining(typeCounterEl, remainingEl, team, TYPES, TYPE_LABEL, TYPE_COLOR){
  ...
  TYPES.map(([k])=>k).filter(k=>!have.has(k)).forEach(k=>{
    const el=document.createElement('span'); el.className='pill';
    el.innerHTML=`<span class="dot" style="background:${TYPE_COLOR[k]}"></span>${TYPE_LABEL[k]}`;
    remainingEl.appendChild(el);
  });
  typeCounterEl.textContent = `고유 속성: ${have.size}/12`;
}
